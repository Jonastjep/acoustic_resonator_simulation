...
%%%%%%%%%%%%%%%%%%%%%%% USER CHOICES %%%%%%%%%%%%%%%%%%%%%%%
extra_name = 'HBAR_coupling_J9_15mK_wide';
doFit = 0
autoSaveFigNoFit = 1;

%fullAuto = 'FA'; semiAuto = 'SA'; manual = 'M'
%fullAuto takes all the set values on the VNA and takes the measurement
%semiAuto mode, you only need to change the span of the measurement (l. 26)
%manual mode requires to enter a start and end freq. (l. 33-34)
mode = 'M';
...
elseif mode == 'SA'
    ...
    centFreq = str2double(query(VNA,'FREQ:CENT?'));
    fstart = round(centFreq- span/2);
    fend =   round(fstart+span);

else
    numpoints = 100001;
    fstart = 4e9;
    fend = 8e9;
    centFreq = (fstart+fend)/2;
    span = fend - fstart;
end
fclose(VNA);
...
folder = ['C:\matlabdata\' num2str(year) '\' num2str(month,'%02d')];
...
folder_pers = ['\\TW-PHYS.org.aalto.fi\PROJECT\nems\Jonas\matlabdata\' num2str(year) '\' num2str(month,'%02d')];
...
for pwrind = 1:length(powers)
    ...
    [ddatamag,ddata,sweep_time]=get_timetrace_ZVB8c(VNA_gpib_adress,fstart, fend,powers(pwrind),averages(pwrind),numpoints,BW);
